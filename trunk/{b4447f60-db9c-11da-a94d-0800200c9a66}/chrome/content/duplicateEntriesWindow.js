...
if(typeof(DuplicateContactsManager_Running) == "undefined") {
	var DuplicateContactsManager_Running = {
		g_DuplicateContactsManagerBundle: null,
		g_addressBookFields: new Array(
			"DisplayName",
			"FirstName",
			"LastName",
			"PhoneticFirstName",
		...
		DuplicateContactsManagerDuplicateManager: {
			 ...
			 */
			init: function() {
				DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle = document.getElementById("bundle_duplicateContactsManager");
				this.running = true;
		...
		//				deleteCard.editCardToDatabase(this.directory.directoryProperties.URI);
				...
				while (!this.foundDuplicate && !this.finished && count < 1000) {	// execute 1000 several steps per interval
					...
					*/
					if (cardsmatch || mailmatch || namesmatch) {
						...
						if (mailmatch) {
							this.statustext.setAttribute('value', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString('matchingEmailAddresses'));
						}
						else {
							this.statustext.setAttribute('value', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString('matchingNames'));
						}
			...
			endSearch: function() {
				...
				this.window.removeAttribute('wait-cursor');
				//this.progresstext.setAttribute('value', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString('finished'));
				this.statustext.setAttribute('value', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString('finished'));
				// hide intro info
				...
				document.getElementById('cancelbutton').setAttribute('label', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString('QuitButtonLabel'));
			...
			displayCardData: function(index1, index2) {
				...
				for (var i=0; i<DuplicateContactsManager_Running.g_addressBookFields.length; i++) {
					// only display if at least one value is present
					var leftField = card1.getProperty(DuplicateContactsManager_Running.g_addressBookFields[i], "");
					var rightField = card2.getProperty(DuplicateContactsManager_Running.g_addressBookFields[i], "");
					if (
						(
						(DuplicateContactsManager_Running.g_addressBookFields[i] != 'LastModifiedDate') && 
						(leftField || rightField) &&
						// not both cards have unknown Mail Format - in this case, the field gets ignored
						!(DuplicateContactsManager_Running.g_addressBookFields[i]=='PreferMailFormat' && leftField=='unknown' && rightField=='unknown')
						) ||
						(DuplicateContactsManager_Running.g_addressBookFields[i]=='SecondEmail' && displaySecondMail)
						) {
						...
						this.displayedFields[fieldCount] = DuplicateContactsManager_Running.g_addressBookFields[i];
						fieldCount++;
				
						labelcell.setAttribute('label', DuplicateContactsManager_Running.g_DuplicateContactsManagerBundle.getString(DuplicateContactsManager_Running.g_addressBookFields[i] + '_label') + ':');
						...
						if (0) {
						// if (DuplicateContactsManager_Running.g_addressBookFields[i] == 'PreferMailFormat') {	// this doesn't work yet
						...
						cell1textbox.setAttribute('id', 'left_'+DuplicateContactsManager_Running.g_addressBookFields[i]);
						cell2textbox.setAttribute('id', 'right_'+DuplicateContactsManager_Running.g_addressBookFields[i]);
				
						if (0) {
						//if (DuplicateContactsManager_Running.g_addressBookFields[i] != 'PreferMailFormat') {
							// nothing
						}
						else {
						...
						cell1.setAttribute('id', 'cell_left_'+DuplicateContactsManager_Running.g_addressBookFields[i]);
						//cell2.appendChild(cell2applybutton);
						cell2.appendChild(cell2textbox);
						cell2.setAttribute('id', 'cell_right_'+DuplicateContactsManager_Running.g_addressBookFields[i]);
